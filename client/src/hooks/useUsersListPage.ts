...
const useUsersListPage = () => {
  ...
  useEffect(() => {
    ...
    const removeUserFromList = (prevUserList: User[], user: User) => {
      const filteredUserList = prevUserList.filter(u => u.username !== user.username);
  ...
  }, [socket, userFilter, userList]); // This actively updates the user list when user is added or deleted

  // case insensitive filter
  const filteredUserlist: User[] = userList.filter(user =>
    user.username.toLowerCase().includes(userFilter.toLowerCase()),
  );
  return { filteredUserlist, setUserFilter };
};
