...
describe('Test userController', () => {
  ...
  describe('GET /getUsers', () => {
    ...
    it('should return an error if there is an error while searching the database', async () => {
      getUsersListSpy.mockResolvedValueOnce({ error: 'Error finding documents' });
      const response = await supertest(app).get(`/user/getUsers`);
  ...
  describe('PATCH /updateBiography', () => {
    ...
    it('should return 400 if user body is invalid', async () => {
      const mockReqBody = {
        username: mockUser.username,
        biography: '',
      };
      ...
      expect(response.status).toBe(400);
      expect(response.text).toEqual('Invalid biography update request');
    });

    // Added tests for updateBiography
    it('should return 500 if there is an error while updating the biography', async () => {
      const mockReqBody = {
        username: mockUser.username,
        biography: 'This is my new bio',
      };
